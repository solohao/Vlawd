...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v2.2 | 2026-07-17 | 明确旗舰 Dogfood 垂直＝创始人自用「考研学习搭子」（MVP.DOGFOOD）：作为 Golden Path 与 MVP Gate 七天使用的具体承载；优先级高于 Excel/Word 等 TOOL.* 日常任务，因其落在 Cycle 1–3 只读研究闭环内、可验证性更强 |
...
### 3.1 旗舰 Dogfood 垂直：考研学习搭子（MVP.DOGFOOD）

Golden Path 需要一个**创始人每天真实使用**的具体任务来承载，本项目选定为「考研学习搭子」（详见 产文/02 §MVP.DOGFOOD）。它不是新增功能，而是把上面三个 Cycle 落到一个真实垂直：

```text
Cycle 1（全双工入口）  → 用语音和 AI 讨论、答疑、被随时插话纠正
Cycle 2（只读研究）    → 搜集考研政策/院校/专业信息，带来源整理
Cycle 3（Session 延续）→ 记录学习进度、连续天数、鼓励；第二天继续
```

优先级说明：该垂直**优先级高于** Excel/Word 等日常任务（后者属技文/11 `TOOL.*`，Post-MVP，需写入型工具护城河）。原因是学习搭子**几乎全为只读研究 + 对话 + 记录**，落在当前 Cycle 1–3 内即可闭环，**可验证性更强**，是最合适的 MVP Gate 七天使用载体。登记为使用场景与验证优先级，不改变各 Cycle 既定工程范围。
