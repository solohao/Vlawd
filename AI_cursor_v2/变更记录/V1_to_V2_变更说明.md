...
## 变更日期：2026-06-17（2026-06-26 修正模型候选定位）
...
```text
1. 全双工语音模型成熟
   BayLing-Duplex / PersonaPlex 等模型进入可验证阶段
   GLM-4-Voice 作为中文语音组件/基座参考，不再作为 native full-duplex 主候选
...
选用：BayLing-Duplex（中文实时监督优先）/ PersonaPlex（英文低延迟验证）
原因：替代 VAD turn-taking pipeline，原生或接近原生支持同时听说、打断/纠正
```
