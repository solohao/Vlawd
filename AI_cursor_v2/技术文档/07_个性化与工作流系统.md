...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v1.0 | 2026-06-01 | 初版：Preference、PhraseAlias、BehaviorRule、WorkflowShortcut 和可监督配置协议 |
...
### WorkflowShortcut（PERSON.WORKFLOW）
...
### SafetyPolicyOverride（PERSON.SAFETY）
...
## 工作流生成与执行（PERSON.WFGEN）
 
工作流不是用户预先配置出来的，而是从任务留痕中生长出来。
...
**Personalization Layer 让 AI 光标能够学习用户的口头暗号、操作偏好和安全边界；Workflow 系统把重复成功路径沉淀为可复用流程，但所有配置都必须通过 propose → confirm → apply → trace 的可监督路径，不能允许 AI 静默自我改造。**